// Auto-generated by scripts/generate-exports.mjs — do not edit manually.
// Source: countries/ee/current/*.pem

export const EE_CERTIFICATION_CENTRE_ROOT_CA = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

export const EE_GOVCA2018 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

export const EEGOVCA2025 = `-----BEGIN CERTIFICATE-----

QqKfvqTFB3eNRnMycNcnJ3rmGe37u9zgH8wnQUuMhUClOGxeRcK4NV9I
-----END CERTIFICATE-----`

export const EID_SK_2016 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

export const ESTEID_SK_2015 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

export const ESTEID2018 = `-----BEGIN CERTIFICATE-----

mT59rZY/2/OeE163BAwD0UdUQAnMPP6+W3Vd
-----END CERTIFICATE-----`

export const ESTEID2025 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

export const KLASS3_SK_2016 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

export const SK_ID_SOLUTIONS_EID_Q_2021E = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

export const SK_ID_SOLUTIONS_EID_Q_2021R = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

export const SK_ID_SOLUTIONS_EID_Q_2024E = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

export const SK_ID_SOLUTIONS_EID_Q_2024R = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

export const SK_ID_SOLUTIONS_ORG_2021E = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

export const SK_ID_SOLUTIONS_ORG_2021R = `-----BEGIN CERTIFICATE-----

aFpWsvg083GHz1gNiikXYeUwygZmqqOj8tbX+thcE+XU/HSLWlHR
-----END CERTIFICATE-----`

export const SK_ID_SOLUTIONS_ROOT_G1E = `-----BEGIN CERTIFICATE-----

XoX3gKSTNpCuZEeamEYBzd8HKLDnKYWV
-----END CERTIFICATE-----`

export const SK_ID_SOLUTIONS_ROOT_G1R = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`

/** All certificates for this country, in alphabetical order. */
export const ALL_CERTS = [
  { name: 'EE-Certification-Centre-Root-CA', exportName: 'EE_CERTIFICATION_CENTRE_ROOT_CA', pem: EE_CERTIFICATION_CENTRE_ROOT_CA },
  { name: 'EE-GovCA2018', exportName: 'EE_GOVCA2018', pem: EE_GOVCA2018 },
  { name: 'EEGovCA2025', exportName: 'EEGOVCA2025', pem: EEGOVCA2025 },
  { name: 'eid-sk-2016', exportName: 'EID_SK_2016', pem: EID_SK_2016 },
  { name: 'esteid-sk-2015', exportName: 'ESTEID_SK_2015', pem: ESTEID_SK_2015 },
  { name: 'esteid2018', exportName: 'ESTEID2018', pem: ESTEID2018 },
  { name: 'esteid2025', exportName: 'ESTEID2025', pem: ESTEID2025 },
  { name: 'klass3-sk-2016', exportName: 'KLASS3_SK_2016', pem: KLASS3_SK_2016 },
  { name: 'sk-id-solutions-eid-q-2021e', exportName: 'SK_ID_SOLUTIONS_EID_Q_2021E', pem: SK_ID_SOLUTIONS_EID_Q_2021E },
  { name: 'sk-id-solutions-eid-q-2021r', exportName: 'SK_ID_SOLUTIONS_EID_Q_2021R', pem: SK_ID_SOLUTIONS_EID_Q_2021R },
  { name: 'sk-id-solutions-eid-q-2024e', exportName: 'SK_ID_SOLUTIONS_EID_Q_2024E', pem: SK_ID_SOLUTIONS_EID_Q_2024E },
  { name: 'sk-id-solutions-eid-q-2024r', exportName: 'SK_ID_SOLUTIONS_EID_Q_2024R', pem: SK_ID_SOLUTIONS_EID_Q_2024R },
  { name: 'sk-id-solutions-org-2021e', exportName: 'SK_ID_SOLUTIONS_ORG_2021E', pem: SK_ID_SOLUTIONS_ORG_2021E },
  { name: 'sk-id-solutions-org-2021r', exportName: 'SK_ID_SOLUTIONS_ORG_2021R', pem: SK_ID_SOLUTIONS_ORG_2021R },
  { name: 'SK-ID-Solutions-ROOT-G1E', exportName: 'SK_ID_SOLUTIONS_ROOT_G1E', pem: SK_ID_SOLUTIONS_ROOT_G1E },
  { name: 'SK-ID-Solutions-ROOT-G1R', exportName: 'SK_ID_SOLUTIONS_ROOT_G1R', pem: SK_ID_SOLUTIONS_ROOT_G1R }
]

...
describe('cross-country integrity', () => {
  it('total trust store carries the small-country anchors plus the TSL sets (>= 360)', () => {
    // CR 10 + BR 4 + AR 2 + ES 2 + EE 16 + PE 8 = 42 fixed, plus the
    // TSL-driven sets: Italy (~231) and Germany (~101). The TSL sets are
    // dynamic (AgID / BNetzA lists), so this is a floor tripwire rather than
    // an exact count.
    const fixed =
      cr.ALL_CERTS.length +
      br.ALL_CERTS.length +
    ...
    const total = fixed + italy.ALL_CERTS.length + de.ALL_CERTS.length
    assert.ok(total >= 360, `Expected >= 360 total, got ${total}`)
  })
